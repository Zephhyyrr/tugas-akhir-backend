...
export async function getDraftUserController(req: Request, res: Response<ResponseApiType>) {
    try {
        const page = Number(req.query.page) || 1;
        const limit = Number(req.query.limit) || 10;

        const result = await getDraftUserService(page, limit);

        return res.status(200).json({
            success: true,
            message: "Berhasil mendapatkan daftar user draft.",
            data: result.data,
            meta: result.meta
        });
    } catch (error) {
