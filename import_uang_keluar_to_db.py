...
# 2. Path ke file Excel PENGELUARAN Anda
EXCEL_FILE_PATH = r"D:\Laporan Kuliah Semester 8\CODE\Backend\Rekapitulas_KAS_Uang_Keluar_Clean.xlsx" 
...
def import_data():
    if not os.path.exists(EXCEL_FILE_PATH):
        ...
        print("Pastikan nama file benar dan sesuaikan path-nya di script.")
        return
    ...
    for index, row in df.iterrows():
        ...
        # Karena ini file khusus pengeluaran, tipenya pasti uang_keluar
        tipe_transaksi = 'uang_keluar'

        # Cari nilai nominal secara dinamis dari kolom yang mungkin ada di Excel
        if 'KREDIT' in df.columns and not pd.isna(row.get('KREDIT')):
            ...
            # Fallback jika kebetulan angka ada di kolom DEBET walaupun ini pengeluaran
            nominal = float(row.get('DEBET', 0))

        if nominal <= 0:
            continue

        # Logika khusus uang keluar: kredit terisi, debit 0
        debit_val = 0
        kredit_val = nominal
    ...
    if not data_to_insert:
        print("Tidak ada data pengeluaran valid yang dapat dimasukkan.")
        return

    print(f"Ditemukan {len(data_to_insert)} baris transaksi pengeluaran. Memulai proses insert ke database...")
    ...
    try:
        ...
        conn.commit()
        print("✅ Data Pengeluaran berhasil dimasukkan ke database!")
    except Exception as e:
        conn.rollback()
