...
# 2. Path ke file Excel Kas Anak Yatim - Uang Keluar
#    Ganti nama file berikut sesuai nama file Excel Anda yang sudah ditempatkan di folder Backend
EXCEL_FILE_PATH = r"D:\Laporan Kuliah Semester 8\CODE\Backend\Rekapitulasi_Yatim_Keluar_Clean.xlsx"

# 3. Konfigurasi Default ID
USER_ID = 1               # ID User (Admin) di tabel User
JENIS_KAS_ID = 2          # ID "Kas Anak Yatim" di tabel JenisKas
...
def import_data():
    ...
    for index, row in df.iterrows():
        ...
        data_to_insert.append((
            ...
            USER_ID,               # userId
            JENIS_KAS_ID,          # jenisKasId (2 = Kas Anak Yatim)
    ...
    try:
        ...
        conn.commit()
        print(f"✅ Berhasil! {len(data_to_insert)} data Uang Keluar Kas Anak Yatim dimasukkan ke database.")
    except Exception as e:
        conn.rollback()
...
if __name__ == "__main__":
    print("=" * 55)
    print("  IMPORT UANG KELUAR - KAS ANAK YATIM (jenisKasId=2)")
    print("=" * 55)
    import_data()
